...
export const BIN2HEX = (number, places) => {
  // Return error if number is not binary or contains more than 10 characters (10 digits)
  if (!isValidBinaryNumber(number)) {
    return error.num;
  }
...
export const BIN2OCT = (number, places) => {
  // Return error if number is not binary or contains more than 10 characters (10 digits)
  if (!isValidBinaryNumber(number)) {
    return error.num;
  }
...
export const BITAND = (number1, number2) => {
...
export const BITLSHIFT = (number, shift) => {
...
export const BITOR = (number1, number2) => {
  number1 = utils.parseNumber(number1);
  number2 = utils.parseNumber(number2);
  if (utils.anyIsError(number1, number2)) {
    return error.value;
...
export const BITRSHIFT = (number, shift) => {
  number = utils.parseNumber(number);
  shift = utils.parseNumber(shift);
  if (utils.anyIsError(number, shift)) {
    return error.value;
...
export const BITXOR = (number1, number2) => {
  number1 = utils.parseNumber(number1);
  number2 = utils.parseNumber(number2);
  if (utils.anyIsError(number1, number2)) {
    return error.value;
...
export const COMPLEX = (real, imaginary, suffix) => {
...
export const CONVERT = (number, from_unit, to_unit) => {
  number = utils.parseNumber(number);
...
export const DEC2BIN = (number, places) => {
  number = utils.parseNumber(number);
  if (number instanceof Error) {
    return number;
  }
...
export const DEC2HEX = (number, places) => {
  number = utils.parseNumber(number);
  if (number instanceof Error) {
    return number;
  }
...
export const DEC2OCT = (number, places) => {
  number = utils.parseNumber(number);
  if (number instanceof Error) {
    return number;
  }
...
export const DELTA = (number1, number2) => {
...
export const ERF = (lower_bound, upper_bound) => {
  // Set number2 to zero if undefined
...
// TODO
exports.ERFC.PRECISE = () => {
  throw new Error('ERFC.PRECISE is not implemented');
};

export const GESTEP = (number, step) => {
  step = step || 0;
  number = utils.parseNumber(number);
...
export const HEX2BIN = (number, places) => {
...
export const HEX2OCT = (number, places) => {
  // Return error if number is not hexadecimal or contains more than ten characters (10 digits)
  if (!/^[0-9A-Fa-f]{1,10}$/.test(number)) {
    return error.num;
  }
...
export const IMAGINARY = (inumber) => {
...
export const IMARGUMENT = (inumber) => {
  // Lookup real and imaginary coefficients using exports.js [http://formulajs.org]
  const x = IMREAL(inumber);
  const y = IMAGINARY(inumber);
...
export const IMCONJUGATE = (inumber) => {
  // Lookup real and imaginary coefficients using exports.js [http://formulajs.org]
  const x = IMREAL(inumber);
  const y = IMAGINARY(inumber);
...
export const IMDIV = (inumber1, inumber2) => {
  // Lookup real and imaginary coefficients using Formula.js [http://formulajs.org]
...
export const IMPOWER = (inumber, number) => {
  number = utils.parseNumber(number);
  const x = IMREAL(inumber);
  const y = IMAGINARY(inumber);
...
export const IMPRODUCT = function () {
  // Initialize result
...
export const IMSUB = function (inumber1, inumber2) {
  // Lookup real and imaginary coefficients using Formula.js [http://formulajs.org]
...
export const IMTAN = function (inumber) {
  // Return error if inumber is a logical value
  if (inumber === true || inumber === false) {
    return error.value;
  }
...
export const OCT2BIN = (number, places) => {
  // Return error if number is not hexadecimal or contains more than ten characters (10 digits)
...
export const OCT2HEX = (number, places) => {
  // Return error if number is not octal or contains more than ten characters (10 digits)
  if (!/^[0-7]{1,10}$/.test(number)) {
    return error.num;
  }
